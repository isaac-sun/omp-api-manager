...
已实现：安装发现、目录解析、只读配置检查 UI、语义 YAML 读写事务、冲突检测、备份、OMP 16 适配器、Keychain 封装、经过验证的 Keychain 服务商草稿、OpenAI/Anthropic 模型发现与连接测试、SSE 网关、脱敏 SQLite 用量持久化、用量仪表盘/导出，以及单元/集成测试。
