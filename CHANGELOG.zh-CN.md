...
- 修复下载版应用启动时立即闪退的问题：发布版现在从标准应用资源目录读取图标，不再调用不可用的 SwiftPM `Bundle.module` 路径。
- 重新设计服务商和模型表单：使用完整宽度的可编辑输入框、淡色示例、字段说明和清晰的行间分隔。
- Release 打包现在会在创建 DMG 前实际启动完成的 `.app` 进行冒烟测试。
