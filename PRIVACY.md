...
The app checks for updates only when the user chooses **Check for Updates…**. This sends a standard unauthenticated HTTPS request to GitHub containing the app version and normal network metadata such as the user's IP address and request time. It does not send a device identifier, OMP configuration, provider/model information, API keys, Keychain contents, or usage records. GitHub processes the request under its own privacy policy.
